...
import { KeyRound, Lock, RotateCcw, ArrowRight, Loader2, CheckCircle2, AlertCircle } from "lucide-react";
...
function ResetPasswordContent() {
  ...
  const onSubmit = async (data: ResetPasswordFormData) => {
    ...
    try {
      ...
      setErrorMessage(err.message || "Failed to reset password. Please verify the code and try again.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="w-full bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-md p-7 sm:p-9 md:p-10 shadow-sm transition-colors duration-300">
        ...
        <div className="text-center space-y-2 mb-6">
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1 max-w-sm mx-auto leading-relaxed">
            Enter your 6-digit recovery code and choose a new, strong password.
          </p>
        </div>
            ...
            <div className="p-4 rounded-md bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 text-emerald-800 dark:text-emerald-300 text-sm flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-700 dark:text-emerald-400 mt-0.5">
                  You can now sign in to your GoMatric account with your updated password.
                </p>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                required
                className="w-full h-11 px-3.5 rounded-md border border-slate-200 dark:border-slate-700 bg-[#F8FAFC]/70 dark:bg-slate-800/60 text-sm text-slate-900 dark:text-white outline-none focus:border-[#061474] dark:focus:border-blue-500 focus:bg-white dark:focus:bg-slate-800"
              />
            </div>
